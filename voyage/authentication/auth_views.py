...
def Contact_us(request):
    if request.method == 'POST':
        ...
        send_mail(subject='Contact us', from_email='nalanda3306@gmail.com', recipient_list=admins,
                  html_message='<h4>from:{}</h4><br><h4>to:cse190001033@iiti.ac.in</h4><br><h3>{}</h3>'.format(email, message), message=message)
        ...
        if request.method == 'POST':
            ...
            if request.session.get('otp') != None:
                ...
                if otp == otp_from_email:
                    ...
                    cursor.execute("""INSERT INTO users(firstname,lastname,gender,address,DOB,mobileno,email,password) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)""", (
                        firstname, lastname, gender, address, DOB, mobileno, email, password))
                    messages.success(
